    ...
    {if $type!=26}
        {foreach from=$rs item=field key=k}
              <tr>
                  ...
                  <td>{$field.CONTENT_CONTENT}</td>
                  <td>{$field.CONTENT_DATE}</td>
                  <td></td>
                  <td><input type="button" value="修改" onclick="location.href='admin.other.resources_new.php?type={$type}&id={$field.CONTENT_ID}'"></td>
                  <td><input type="button" value="刪除" onclick="location.href='admin.other.resources_new.php?process=3&type={$type}&id={$field.CONTENT_ID}'"></td>
              </tr>
         {/foreach}
    {else}
    	  {foreach from=$rs item=field key=k}
