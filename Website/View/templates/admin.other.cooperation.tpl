    ...
    {if $type==30}
       {foreach from=$rs item=field key=k}
              <tr>
                  ...
                  <td></td>
                  <td><input type="button" value="修改" onclick="location.href='admin.other.cooperation_new.php?type={$type}&id={$field.CONTENT_ID}'"></td>
                  <td><input type="button" value="刪除" onclick="location.href='admin.other.cooperation_new.php?process=3&type={$type}&id={$field.CONTENT_ID}'"></td>
              </tr>
        {/foreach}
    {else}
    	{foreach from=$rs item=field key=k}
